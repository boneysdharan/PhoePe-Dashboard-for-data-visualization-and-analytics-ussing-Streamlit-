...
#st.title("Top User Data - State-wise (India Map)")
...
top_10_df = (
    Top_user_state.groupby("State")["Registered_users"]
    .sum()
    .reset_index()
...
# Show Data Preview
"""
if st.checkbox("Show Raw Data", key="raw_data_top_user_top10"):
...
bottom_10_df = (
    ...
    .sum()
    .reset_index()
    .sort_values(by="Registered_users", ascending=True)
    .head(10))
bottom_10_df = bottom_10_df.reset_index(drop=True)  # Drop old index
bottom_10_df.index += 1
# Show Data Preview
"""
if st.checkbox("Show Raw Data", key="raw_data_top_user_bottom10"):
    st.subheader("Least 10 States by Registered User Count")
    st.dataframe(bottom_10_df)
"""
# Load GeoJSON data from URL
# Plotting Total Registered_Users count by State
state_summary = (
    Top_user_state
    .groupby('State', as_index=False)
    .agg({'Registered_users': 'sum'})
)

Top_User_fig = px.choropleth(
    state_summary,
    ...
    title='Total Registered_Users count by State'
)

Top_User_fig.update_geos(fitbounds="locations", visible=False)
